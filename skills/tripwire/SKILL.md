---
name: tripwire
description: Catches the user (not the agent) walking into a complexity or pain trap and flags it before the work starts, then lets the user decide. Floor 1 uses the user's own profile of recurring traps — another layer, guard, reviewer or research round, automation before a problem repeats, architecture before the minimal version failed, a new thread before the current one is done, a closed decision reopened, scaling before it works for one person. Floor 2 is a quieter general catalog of engineering pain — sunk cost, planning fallacy, second-system effect, one green run as proof, bikeshedding, yak shaving. Use whenever the user proposes a new guard, check, agent, automation, framework or platform, says "while this runs, let's also…", keeps pushing a stalled path, estimates, or declares something done; also for a roast, a self-check, "am I overengineering this?", or a "tripwire review".
---

# Tripwire

Scope guards stop the *agent* from building too much. Tripwire watches the other side: the *user's* moves that lead into building too much or into avoidable pain. It never blocks. It flags the trap at the moment it opens, and the user decides.
